...
\item
  ...
  strategy}.
\item
  The number of correct guesses of a randomization sequence is then
  \(CG(t) := \sum_{i=1}^{N-1} g(t_{i+1})\), and the proportion of
  correct guesses is \(CG(t)/N\).
\end{itemize}
...
\begin{center}\includegraphics[height=0.8\textheight]{./graphics/correct-guesses} \end{center}
\end{frame}
